<%--
  Created by IntelliJ IDEA.
  User: geovannadiniz-ieg
  Date: 02/11/2023
  Time: 18:44
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Alterar</title>
    <link rel="stylesheet" href="../style.css">

</head>
<body>
<img src="../assets/3rings.png" alt="Style Ring" class="ThreeRings">
<header>
    <a href="https://mobisi.onrender.com/"><img class="logo" src="../assets/logoComTexto.png" alt="Logo Mobisi"></a>
    <nav class="headerNav">
        <a href="../../adm.html"><button>Tabela</button></a>
        <a href="crudEst.html"><button>CRUD</button></a>
    </nav>
</header>
<main>
    <section class="section1">
        <h1>Alterar </h1>
        <h2>Tabela Estabelecimento</h2>

        <form action="alterarEst" method="post">
            <label for="id" >Id</label>
            <input type="text" name="id" id="id" value="<%=request.getAttribute("id")%>" >
            <br>
            <label for="nome">Insira o nome</label>
            <input type="text" name="nome" id="nome" value="<%=request.getAttribute("nome")%>">
            <br>
            <label for="nota">Insira a nota</label>
            <input type="text" name="nota" id="nota" value="<%=request.getAttribute("nota")%>">
            <br>
            <label for="cidade">Insira a cidade</label>
            <input type="text" name="cidade" id="cidade" value="<%=request.getAttribute("cidade")%>">
            <br>
            <label for="estado">Insira o estado</label>
            <input type="text" name="estado" id="estado" value="<%=request.getAttribute("estado")%>">
            <br>
            <label for="cep">Insira o cep</label>
            <input type="text" name="cep" id="cep" value="<%=request.getAttribute("cep")%>">
            <br>
            <label for="rua">Insira a rua</label>
            <input type="text" name="rua" id="rua" value="<%=request.getAttribute("rua")%>">
            <br>
            <label for="tpEst">Insira o id tipo Estabelecimento</label>
            <input type="text" name="tpEst" id="tpEst" value="<%=request.getAttribute("tpEst")%>">
            <br>
            <input type="submit" value="Enviar" id ="enviar">
        </form>

    </section>
</main>
</body>
</html>
